...
Assigning a different constraint set uses the same sequence as the manual safe workflow: `ConstraintsTool` calls the Animation Preview controller's full Stop operation, processes the selected set, then presses Play programmatically only when playback had been running before the change. The connection is wired directly by `SkinningModule` for the Position, Rotation, and Scale tools; it does not alter AnimationClip bone paths or add a second post-load rebind.

When applying Sprite Editor changes, bones are serialized in parent-before-child order. This prevents generated constraint parents from being saved after their Driven children and avoids Unity rebuilding saved bone positions or rotations from forward parent references. Mesh vertex weights are remapped from the editor mesh bone list, which excludes generated constraint parents, into the serialized SpriteBone list that includes them. Character-skeleton and sprite-skeleton bone objects are matched by GUID during this conversion because the Character Part influence list can use a different order. Serialized `CharacterPart.bones` is rebuilt in the same parent-first SpriteBone order so PSD Importer generates a matching runtime `SpriteSkin.boneTransforms` array. When the cache is loaded again, saved weights are first interpreted against the complete SpriteBone list and then remapped into the filtered mesh-compatible bone list. This keeps weights stable across Apply/reload when service bones occur before weighted bones.
